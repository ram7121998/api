...
export const storeReport = async (req, res) => {
    try {
        ...
        const errors = {};
        if (!reported_to_id)
            errors.reported_to_id = "reported_to_id is required";
        if (!trade_id)
            errors.trade_id = "trade_id is required";
        if (!reason)
...
export const getUsersReport = async (req, res) => {
    try {
        ...
        const pagination = {
            ...
            last_page_url: `${req.protocol}://${req.get("host")}${req.path}?page=${Math.ceil(totalFilteredReport / perPage)}`,
            next_page_url: page < Math.ceil(totalFilteredReport / perPage)
