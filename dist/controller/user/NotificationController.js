...
export const getAllNotifications = async (req, res) => {
    try {
        ...
        const unreadNotifications = unreadUser + Number(unreadGlobal[0]?.count || 0);
        // ─────────────────────────────────────────────
        // 3️⃣ MAIN QUERY
        // ─────────────────────────────────────────────
        let where = {
        ...
        // 4️⃣ FORMAT DATA
        // ─────────────────────────────────────────────
        const data = await Promise.all(notifications.map(async (n) => {
            let is_read;
            if (n.user_id) {
                is_read = n.is_read;
            }
...
export const getPerticularNotification = async (req, res) => {
    try {
        ...
        // ==============================
        const cipher = crypto.createCipheriv("aes-256-ecb", Buffer.from(process.env.ENCRYPTION_KEY, "utf8"), null);
