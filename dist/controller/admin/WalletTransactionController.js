...
export const getWalletDetails = async (req, res) => {
    try {
        ...
        const { user_id, blockchain, network, asset, wallet_address, status, per_page, page, } = req.query;
        const perPage = parseInt(per_page) || 10;
        const currentPage = parseInt(page) || 1;
        const skip = (currentPage - 1) * perPage;
        let whereClause = {};
        if (user_id)
            whereClause.user_id = BigInt(user_id);
        if (blockchain)
            whereClause.blockchain = blockchain;
        if (network)
            whereClause.network = network;
        if (asset)
            whereClause.asset = asset;
        if (wallet_address)
            whereClause.wallet_address = wallet_address;
        if (status)
...
export const getTransactionDetails = async (req, res) => {
    try {
        ...
        const { user_id, transaction_type, network, asset, txn_hash, method, status, start_date, end_date, per_page, page, } = req.query;
        const perPage = parseInt(per_page) || 10;
        const currentPage = parseInt(page) || 1;
        const skip = (currentPage - 1) * perPage;
        // where clause
        ...
        if (user_id)
            whereClause.user_id = BigInt(user_id);
        if (transaction_type)
            whereClause.txn_type = transaction_type;
        if (network)
            whereClause.network = network;
        if (asset)
            whereClause.asset = asset;
        if (txn_hash)
            whereClause.txn_hash_id = txn_hash;
        if (method)
            ...
            if (status === "success") {
                ...
                await tx.admin_assets.update({
                    ...
                    data: {
                        ...
                        available_balance: Number(admin.total_deposit) -
