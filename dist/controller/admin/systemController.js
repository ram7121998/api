...
export const updateSettingData = async (req, res) => {
    ...
    try {
        const { withdraw_status, deposit_status, withdraw_type, min_withdraw, max_withdraw, user_registration, } = req.body;
        // ✅ Validation
        ...
        const updatedSetting = await prisma.settings.update({
            where: { setting_id: 1 },
            data: {
                ...Object.fromEntries(Object.entries(updatableFields).filter(([_, v]) => v !== undefined)),
...
export const changeEmailCredential = async (req, res) => {
    try {
        const { mail_mailer, mail_host, mail_port, mail_username, mail_password, mail_encryption, mail_from_address, mail_from_name } = req.body;
        // Convert port to int
        if (mail_port)
            req.body.mail_port = parseInt(mail_port);
        ...
        if (mail_mailer)
            setEnvValue("MAIL_MAILER", mail_mailer, false);
        if (mail_host)
            setEnvValue("MAIL_HOST", mail_host, false);
        if (mail_port)
            setEnvValue("MAIL_PORT", mail_port, false);
        if (mail_username)
            setEnvValue("MAIL_USERNAME", mail_username, false);
        if (mail_password)
            setEnvValue("MAIL_PASSWORD", mail_password, false);
        if (mail_encryption)
            setEnvValue("MAIL_ENCRYPTION", mail_encryption, false);
